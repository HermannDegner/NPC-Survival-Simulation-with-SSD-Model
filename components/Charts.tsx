...
const chartKeys: ChartType[] = ['hunger', 'E', 'T', 'kappa_forage', 'kappa_hunt'];
const chartToggleLabels: Record<ChartType, string> = {
    hunger: "HUNGER",
    E: "E",
    T: "T",
    kappa_forage: "KAPPA FORAGE",
    kappa_hunt: "KAPPA HUNT"
};
...
const Charts: React.FC<ChartsProps> = ({ logData, npcNames }) => {
    ...
    const chartData = useMemo(() => {
        ...
        const relevantKey = chartConfigs[activeChart].key;
        logData.forEach(entry => {
            if (!dataByTick[entry.t]) {
                dataByTick[entry.t] = { t: entry.t };
            }
            if (entry[relevantKey] !== undefined) {
                 dataByTick[entry.t][entry.name] = entry[relevantKey];
            }
        });
    ...
    }, [logData, activeChart, npcNames, chartConfigs]);
                 ...
                 <div className="flex flex-wrap space-x-1 p-1 bg-gray-700 rounded-md">
                    {chartKeys.map((key) => (
                        <button 
                            ...
                            className={`px-3 py-1 text-xs font-semibold rounded transition-colors ${activeChart === key ? 'bg-cyan-500 text-white' : 'text-gray-300 hover:bg-gray-600'}`}
                        >
                            {chartToggleLabels[key]}
                        </button>
                    ))}
                 </div>
            </div>
                ...
                <ResponsiveContainer>
                    <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                        ...
                        {activeChart === 'hunger' && deathEvents.map(event => (
                            <ReferenceLine key={event.t + event.name} x={event.t} stroke="red" strokeDasharray="3 3">
                               <Label value="†" position="top" fill="red" fontSize={16}/>
                            </ReferenceLine>
                        ))}
                    </LineChart>
                </ResponsiveContainer>
